...
archive_release_artifacts() {
  ...
  if [ -n "$BUILD_NUMBER" ]; then
    DEPLOY_RELEASE_ARTIFACTS_ZIP=coherence-ce-${VERSION_RELEASE}-${BUILD_NUMBER}-release.tar
  else
    DEPLOY_RELEASE_ARTIFACTS_ZIP=coherence-ce-${VERSION_RELEASE}-release.tar
  fi

  tar -cvf $DEPLOY_RELEASE_ARTIFACTS_ZIP -C $HOME/.m2/repository $ALL_RELEASE_ARTIFACTS
  tar -tvf $DEPLOY_RELEASE_ARTIFACTS_ZIP
}
...
publish_api_docs() {
  ...
  cp -r $WORKSPACE/api_out/* $REVISION/$API_JAVA/
  cp -r $WORKSPACE/docs_out/* $REVISION/docs/
  ...
  echo "Creating zip containing generated API for v${REVISION} : ${WORKSPACE}/.archives/coherence-v${REVISION}_api.zip"
  zip -r ${WORKSPACE}/.archives/coherence-v${REVISION}_api.zip ${REVISION}/${API_JAVA}/*
  ls -lh ${WORKSPACE}/.archives/coherence-v${REVISION}_api.zip
}
...
GIT_REPO=https://coherence-bot:${GITHUB_TOKEN}@github.com/${REPO_NAME:-oracle/coherence}
